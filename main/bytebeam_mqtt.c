...
// char CAN_TOPIC[] =    "/tenants/torkdemo/devices/1/events/c2c_can/jsonarray/zlibdeflate";
// char GPS_TOPIC[] =    "/tenants/torkdemo/devices/1/events/c2c_gps/jsonarray";
// char SUB_TOPIC[] =    "/tenants/torkdemo/devices/1/actions";
// char ACTION_TOPIC[] = "/tenants/torkdemo/devices/1/action/status";
...
//     strncpy(paho_mqtt_at_param.clientID, "1", sizeof("1"));
...
//     JSMN_JSON_CAN_MESSAGE_st_obj.can_id = (uint32_t)((payload[2] << 24) | (payload[3] << 16) |
...
//                           //  nwy_ext_echo("\r\nInside for loop\r\n");
//                             uint8_t *p = uart_strcut.payload + i*14;
//                             send_can_mqtt_pack(p, 14,  uart_strcut.timestamp);
//                         }
...
void mqtt_topics_init()
{
    ...
    sprintf(CONTENT_TOPIC, "/%s/%s/%s/%s/%s", "tenants",project_id_global,"devices",device_id_global, "events/content/jsonarray");
       ...
       if (nwy_get_msg_que(mqtt_publish_msg_queue, &uart_strcut, 0) == true)
        {
            ...
            if (MQTTIsConnected(&paho_mqtt_client))
            {
                switch (uart_strcut.cmd)
                {
                    ...
                    {
                        memset(paho_mqtt_at_param.topic, 0, sizeof(paho_mqtt_at_param.topic));
                        strncpy(paho_mqtt_at_param.topic, CONTENT_TOPIC, strlen(CONTENT_TOPIC));
                        paho_mqtt_at_param.qos = 1;
                        paho_mqtt_at_param.retained = 0;
                        memset(paho_mqtt_at_param.message, 0, sizeof(paho_mqtt_at_param.message));
                        strncpy(paho_mqtt_at_param.message, uart_strcut.payload , uart_strcut.length_u16);
