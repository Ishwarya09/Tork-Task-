...
uint8_t json_content_thread_buff_gau8[CONTENT_THREAD_MSG_BUFF_LEN] = {
    0,
};
...
uint8_t json_pack_content_message(JSMN_JSON_CONTENT_MESSAGE_tst *content_mess_pst);
...
uint8_t json_pack_content_message(JSMN_JSON_CONTENT_MESSAGE_tst *content_mess_pst)
{
    ...
    static uint32_t seq_id = 0;
    seq_id++;
    memset(json_content_thread_buff_gau8, 0, JSON_CONTENT_THREAD_BUFF_LEN);
    snprintf((char *)&json_content_thread_buff_gau8, sizeof(json_content_thread_buff_gau8), newaction_mess_json,
             timestamp1,
             content_mess_pst-> content,
             seq_id);
    return 0;
}
#if 0
