...
struct action_functions_map action_funcs[] = { {"update_firmware", handle_ota},
                                               {"Content", handle_content}, {NULL, NULL} };
    ...
    else // if connection exists and space exists send message
    {
        ...
        else
        {
            store_msg_flag = false;     
                //disabled until persistance is implemented
            if(store_queue != NULL)
            {
                if(nwy_get_queue_spaceevent_cnt(store_queue) != 0)
                {
...
int handle_content(char *payload_string, char *action_id)
{
    char content[30] ="Hello_Cloud ";
    if( nwy_lock_mutex(content_thread_mutex, 0) == false)
    {
          nwy_ext_echo("\r\nUnable to trigger new action. Thread is currently busy.\r\n");
    }
    else
    {   
        nwy_ext_echo("\r\nTriggering New action\r\n");
          ...
          // Unlock the mutex
        nwy_unlock_mutex(content_thread_mutex);
        // Call the contentthread function
        content_thread_new(content);

    }

    return 0;
}
void content_thread_new(void *param)
{
     char *action_id = (char *)param;
     char content[30]; 

    if (nwy_lock_mutex(content_thread_mutex, 0xFFFF) == true)
    {
    nwy_ext_echo("%s", content);
    publish_content_threadnew(&content);
    }
}
int publish_content_threadnew(char *content)
{
                gettimeofday(&tv, NULL);
                ...
                seq_id++;
                memset(json_content_thread_buff_gau8, 0, CONTENT_THREAD_MSG_BUFF_LEN);
                snprintf(json_content_thread_buff_gau8, sizeof(json_content_thread_buff_gau8), newaction_mess_json,
                         timestamp1,
                         seq_id,
                         content);
                nwy_ext_echo("%s", json_content_thread_buff_gau8); // For the verification of the JSON frame

                UART_data_struct content_publish_msg;
                content_publish_msg.cmd = 0x987;
                content_publish_msg.length_u16 = strlen((char *)json_content_thread_buff_gau8);

                memcpy(content_publish_msg.payload, json_content_thread_buff_gau8, content_publish_msg.length_u16);
                ...
                if (get_MQTT_COnnection_Status() && (store_msg_flag == false)) 
                {
                    ...
                    else
                    {
                        ...
                        nwy_put_msg_que(mqtt_publish_msg_queue, &content_publish_msg, 0);
                    }
                }
                else
                {
                    ...
                    if (nwy_read_sdcart_status() == true) 
                    {
                        ...
                        nwy_put_msg_que(store_queue, &content_publish_msg, 0);
                    }
                    else 
                    {
