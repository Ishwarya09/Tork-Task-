...
void get_ble_mac_id()
{
    ...
    if (ble_mac != NULL) 
    {
        // char ble_mac_id[25] = {0,};
        // Read the MAC address from the ble_mac buffer
        ...
        // strncpy(ble_mac_id_g, ble_mac_id, sizeof(ble_mac_id_g));
...
void bytebeam_ble()
{
    ...
    // if (!nwy_ble_set_character(0, char_uuid_1, 1))
    //     nwy_ext_echo("\r\n BLE HAS STARTED !\r\n");
    //nwy_ble_set_character(0, char_uuid_1, 1);
    // nwy_ext_echo("\r\n char_prop: 0:Read 1:Read | Notify\r\n");

    // nwy_ble_set_character(1, char_uuid_2, 1);
    // if (!nwy_ble_set_character(1, char_uuid_2, 1))
    //     nwy_ext_echo("\r\n BLE HAS STARTED !\r\n");
...
int appimg_enter(void *param)
{
    ...
    device_config_thread_handle = nwy_create_thread("device_config_thread", DeviceConfig_ThreadEntry, NULL,
                                                    NWY_OSI_PRIORITY_NORMAL, 1024*18, 16);//rtos ...
//    do{
...
//        else
//        {
//            nwy_ext_echo("Config data retrieval failed\r\n");
//            nwy_sleep(3000);
//        }
//    } while((parse_device_config_file() < 0) && (config_data_present_in_nwy == 0));
    //mqtts_cert_add();
    mqtt_topics_init();
    test_imu();
...
// #define BLOCKS_PER_RECORD 20
// #define RECORD_SIZE (BLOCK_SIZE * BLOCKS_PER_RECORD)

// // The maximum size of a file name in a tar archive is 100 bytes
// #define MAX_FILENAME_LENGTH 100

// // The maximum size of a file size in a tar archive is 12 bytes
...
// int untar(const char* tar_filename) 
// {
//   // Open the tar file for reading
//   int tar_fd = nwy_sdk_fopen(tar_filename, NWY_RDONLY);
//   if (tar_fd < 0) 
//   {
//     nwy_ext_echo("Error opening tar file\r\n");
//     return 1;
//   }

//   // Read the tar file in blocks of 512 bytes
//   char block[BLOCK_SIZE];
//   while (nwy_sdk_fread(tar_fd, block, BLOCK_SIZE) == BLOCK_SIZE) 
//   {
//     // Extract the file name from the tar header
//     char filename[MAX_FILENAME_LENGTH + 1];
//     strncpy(filename, block, MAX_FILENAME_LENGTH);
//     filename[MAX_FILENAME_LENGTH] = '\0';

//     // Extract the file size from the tar header
//     char filesize_str[MAX_FILESIZE_LENGTH + 1];
//     strncpy(filesize_str, block + 124, MAX_FILESIZE_LENGTH);
//     filesize_str[MAX_FILESIZE_LENGTH] = '\0';
//     int filesize = strtol(filesize_str, NULL, 8);

//     // Create a new file to store the extracted data
//     int out_fd = nwy_sdk_fopen(filename, NWY_FILE_CREAT | NWY_FILE_WRONLY);
//     if (out_fd < 0) 
//     {
//       fprintf(stderr, "Error creating output file\n");
//       nwy_sdk_fclose(tar_fd);
//       return 1;
//     }
//   }
